...
try:
    sys.path.append("..")
    from dao import article as article_sql
except:
...
class TOUTIAO:
    ...
    def crawl_and_save_single(self, addr):
        print("in crawl_and_save_single ! ")
        ...
        extra = 'tags: %s' % tags
        id_, type_ = article_sql.save_article(title=title, document="",
        ...
        for comment in comments:
            ...
            article_sql.save_review(object_type=id_, object_id=type_,
    ...
    def crawl_and_save(self, keyword, max_news_num, thread_num=10):
        ...
        for num, addr in enumerate(addrs[2:]):
            ...
            while threading.activeCount() > thread_num:
                time.sleep(1)
                print('#')
            Gennral_thread(self.crawl_and_save_single, [addr]).run()
